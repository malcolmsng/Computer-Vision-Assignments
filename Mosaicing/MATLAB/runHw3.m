...
signAcademicHonestyPolicy('Sng Hong Wee Malcolm', 'University of Wisconsin-Madison');
...
[p_x, p_y] = getPointsFromUser(portrait_img, 4, 'Click any 4 points'); 
portrait_pts = [p_x,p_y];
disp("Dab");
% bg_pts = [xb1 yb1; xb2 yb2; xb3 yb3; xb4 yb4];
[bg_x, bg_y] = getPointsFromUser(bg_img, 4, 'Click any 4 points');
bg_pts = [bg_x, bg_y];
...
mask = ~mask;
disp(size(bg_img))
disp(size(cat(3, mask, mask, mask)))
